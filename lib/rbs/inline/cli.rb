    ...
    class CLI
      ...
      def run(args)
        ...
        OptionParser.new do |opts|
          ...
          opts.on("--default-type=TYPE", "Use given type instead of untyped for unknown types (default: untyped)") do
            default_type = _1
          end
        ...
        targets.each do |target|
          ...
          if (uses, decls, rbs_decls = Parser.parse(Prism.parse_file(target.to_s), opt_in: opt_in))
            ...
            if default_type
              type = RBS::Parser.parse_type(default_type, require_eof: true)
              type or raise
              writer.default_type = type
            end
